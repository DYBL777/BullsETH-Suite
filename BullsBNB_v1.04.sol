...
/**
 * @title  BullsBNB v1.04  [contract renamed from NearestTheBNB30_BSC]
 * @notice 30-draw BNB/USD prediction game on BSC. 72h draw cycle, 90-day season.
 ...
 *         STABLECOIN NOTE: Native Circle USDC is NOT available on BSC via Circle CCTP.
 *         Recommended: Binance-Peg USDC (0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d).
 *         Confirm stablecoin selection before mainnet deployment.
 *         FORK CHANGES FROM BullsEth v2.26:
 *           - Price feed: ETH/USD (Base) -> BNB/USD (BSC Mainnet)
 *           - Chain: Base Mainnet -> BSC Mainnet
 *           - Sequencer: address(0) valid for BSC (no L2 sequencer)
 *           - Constructor: _sequencerFeed address(0) guard removed for BSC compat
 *           - All other mechanics, parameters, and audit fixes unchanged
 *         BullsBNB v1.0 audit history begins here.
 *         BullsBNB v1.01: Two stale "30-draw Base fork" inline comments
 *               updated to "this 30-draw game" (Base chain ref not applicable
 *               on BSC).
 *         BullsBNB v1.02: ABI renames: wethFeed->wbnbFeed,
 *               setWethFeed->setWbnbFeed, pendingEthFeedChange->pendingBnbFeedChange,
 *               _readEthPrice->_readBnbPrice, ethReserveFeed->btcReserveFeed.
 *               NS: INHERITED attribution fixed to BullsEth v2.26.
 *               NS: setWbnbFeed @param WETH->WBNB/USD. Brand history restored.
 *         BullsBNB v1.03: All BullsEth v2.27-v2.31 code fixes applied.
 ...
 *               Brand: NearestTheETH30 -> BullsEth. [BullsBNB forked from BullsEth v2.26.]
 ...
 *               NS-I-02 setReserveFeed/setWbnbFeed FeedSubstituted semantics comment.
 ...
 *                       (BSC Mainnet, 8 dec, ~0.5% deviation; confirm heartbeat at docs.chain.link)
 *         USDC: 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d (Binance-Peg USDC)
 *               NOTE: Native Circle USDC not available via CCTP on BSC -- confirm stablecoin.
 *         Sequencer: address(0) -- BSC has no L2 sequencer. Guard in _checkSequencer().
...
contract BullsBNB is ReentrancyGuard, Ownable2Step {
    ...
    ///      in BullsBNB. The qualification threshold is only reachable on the final draw.
    ...
    // [v1.3] MULLIGAN_THRESHOLD removed. No mulligan in BullsBNB 30-draw game.
    ...
    PendingFeedChange public pendingBnbFeedChange;
    ...
    /// @notice Deploys BullsBNB_NoAave. Pure USDC contract -- no Aave dependency.
    /// @param _usdc                  USDC token address. BSC: 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d
    /// @param _bnbFeed               Chainlink BNB/USD feed (8 decimals). BSC: 0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE
    /// @param _defaultPrediction     Default prediction in USD cents for draw-1 auto-default.
    /// @param _sequencerFeed         L2 sequencer uptime feed. Pass address(0) for BSC (no L2 sequencer).6433
    /// @param _protocolBeneficiary   Immutable recipient for all post-game unclaimed sweeps.
    constructor(
        address _usdc, address _bnbFeed,
        uint256 _defaultPrediction, address _sequencerFeed, address _protocolBeneficiary
    ) Ownable2Step() {
        if (_usdc == address(0)) revert InvalidAddress();
        if (_bnbFeed == address(0)) revert InvalidAddress();
        ...
        // [BullsBNB v1.0] address(0) accepted for BSC -- no L2 sequencer on this chain.
        // _checkSequencer() returns immediately when SEQUENCER_FEED == address(0).
        // if (_sequencerFeed == address(0)) revert InvalidAddress(); // removed for BSC
        try AggregatorV3Interface(_bnbFeed).decimals() returns (uint8 dec) {
    ...
    /// @param prediction  BNB/USD price prediction in USD cents.
    ...
    /// @param prediction   First BNB/USD prediction (USD cents).
    /// @param prediction2  Second BNB/USD prediction (USD cents).
    ...
    /// @param prediction   BNB/USD price in USD cents. Primary prediction for all draws.
    ...
    ///         Missing a draw loses status -- no mulligan in BullsBNB.
    ...
    /// @param prediction   First BNB/USD prediction for the current draw (USD cents).
    /// @param prediction2  Second BNB/USD prediction for the current draw (USD cents).
    ...
    /// @notice Sets the reserve BNB/USD price feed used as primary fallback. Owner only.
    ...
    function setReserveFeed(address _reserveFeed) external onlyOwner {
        ...
        if (_reserveFeed == bnbFeed) revert FeedUnchanged();
        if (_reserveFeed == wbnbFeed) revert FeedUnchanged();
    ...
    /// @notice Sets the WBNB/USD price feed used as secondary fallback. Owner only.
    /// @param _wbnbFeed  New WBNB/USD fallback feed address. address(0) clears the fallback.
    function setWbnbFeed(address _wbnbFeed) external onlyOwner {
        // FeedSubstituted reused for admin feed management -- semantically distinct from startGame() usage.
        if (_wbnbFeed == address(0)) { address old = wbnbFeed; wbnbFeed = address(0); emit FeedSubstituted(old, address(0)); return; }
        if (_wbnbFeed == USDC || _wbnbFeed == address(this)) revert InvalidAddress();
        if (_wbnbFeed == SEQUENCER_FEED && SEQUENCER_FEED != address(0)) revert InvalidAddress();
        if (_wbnbFeed == bnbFeed) revert FeedUnchanged();
        if (_wbnbFeed == ethReserveFeed) revert FeedUnchanged();
        try AggregatorV3Interface(_wbnbFeed).decimals() returns (uint8 dec) { if (dec != 8) revert FeedDecimalsMismatch(); } catch { revert FeedDecimalsMismatch(); }
        address oldWeth = wbnbFeed;
        wbnbFeed = _wbnbFeed;
        emit FeedSubstituted(oldWeth, _wbnbFeed);
    }
    ...
    function startGame() external onlyOwner nonReentrant {
        ...
        if (price == 0 && ethReserveFeed != address(0)) {
            ...
            if (rPrice > 0) {
                ...
                if (decimalOk) { emit FeedSubstituted(bnbFeed, ethReserveFeed); bnbFeed = ethReserveFeed; price = rPrice; }
            }
        }
    ...
    /// @param prediction2  Second BNB/USD price prediction in USD cents.
    ...
    /// @notice Resolves BNB price and transitions to CUTOFF_SUBMISSION.
    ...
    function resolveWeek() external nonReentrant {
        ...
        int256 currentPrice = _readBnbPrice();
        if (currentPrice > 0) { lastValidPrice = currentPrice; }
            ...
            if (currentPrice == 0 && wbnbFeed != address(0)) {
                int256 wPrice = _readPriceFeed(wbnbFeed);
                if (wPrice > 0) { currentPrice = wPrice; lastValidPrice = wPrice; emit ReserveFeedUsed(wbnbFeed, currentDraw); }
            }
    ...
    function proposeDormancy() external onlyOwner {
        ...
        if (pendingBnbFeedChange.effectiveTime != 0) { delete pendingBnbFeedChange; emit FeedChangeCancelled(); }
    ...
    /// @param newFeed  Address of the new primary BNB/USD Chainlink feed (8 decimals required).
    function proposeFeedChange(address newFeed) external onlyOwner {
        ...
        if (newFeed == bnbFeed) revert FeedUnchanged();
        if (newFeed == ethReserveFeed) revert FeedUnchanged();
        if (newFeed == wbnbFeed) revert FeedUnchanged();
        if (pendingBnbFeedChange.effectiveTime != 0) revert TimelockPending();
        ...
        pendingBnbFeedChange = PendingFeedChange(newFeed, block.timestamp + TIMELOCK_DELAY);
    ...
    function executeFeedChange() external onlyOwner {
        if (gamePhase != GamePhase.ACTIVE) revert WrongPhase();
        if (drawPhase != DrawPhase.IDLE) revert DrawInProgress();
        if (pendingBnbFeedChange.effectiveTime == 0) revert NoTimelockPending();
        if (block.timestamp < pendingBnbFeedChange.effectiveTime) revert TooEarly();
        address oldFeed = bnbFeed;
        bnbFeed = pendingBnbFeedChange.newFeed;
        lastValidPrice = 0; autoDefaultCents = 0;
        emit FeedChangeExecuted(oldFeed, bnbFeed);
        delete pendingBnbFeedChange;
    }
    ...
    function _continueUnwind() internal {
        ...
        for (uint256 i = start; i < end; i++) {
            ...
            // [v1.3] No mulligan unwind needed -- mulligan removed from BullsBNB.
        }
    ...
    /// @notice Returns the most recently resolved BNB/USD price (Chainlink 8-dec).
    ...
    function _readBnbPrice() internal view returns (int256) {
        AggregatorV3Interface feed = AggregatorV3Interface(bnbFeed);
        try feed.latestRoundData() returns (uint80 roundId, int256 price, uint256, uint256 updatedAt, uint80 answeredInRound) {
            ...
            if (price <= 0) return 0;
            try AggregatorMinMax(bnbFeed).minAnswer() returns (int192 minAns) { if (price <= int256(minAns)) return 0; } catch {}
            try AggregatorMinMax(bnbFeed).maxAnswer() returns (int192 maxAns) { if (maxAns > 0 && price >= int256(maxAns)) return 0; } catch {}
            return price;
        } catch { return 0; }
    }
