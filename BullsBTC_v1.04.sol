...
/**
 * @title  BullsBTC v1.04  [contract renamed from NearestTheBTC30_Arb]
 * @notice 30-draw BTC/USD prediction game on Arbitrum One. 72h draw cycle, 90-day season.
 *         Fork of BullsEth v2.26 (NearestTheETH30_Base, Base Mainnet). All v2.26
 *         audit fixes inherited. See BullsEth_v2.26.sol for full development history.
 *         Native Circle USDC available on Arbitrum (0xaf88d065e77c8cC2239327C5EDb3A432268e5831).
 *         FORK CHANGES FROM BullsEth v2.26:
 *           - Price feed: ETH/USD (Base) -> BTC/USD (Arbitrum One)
 *           - Chain: Base Mainnet -> Arbitrum One
 *           - Sequencer: 0xFdB631F5EE196F0ed6FAa767959853A9F217697D (Arbitrum)
 *           - ethFeed state var renamed to btcFeed
 *           - ethReserveFeed state var renamed to btcReserveFeed
 *           - All other mechanics, parameters, and audit fixes unchanged
 *         BullsBTC v1.0 audit history begins here.
 *         BullsBTC v1.01: Two stale "30-draw Base fork" inline comments
 *               updated to "this 30-draw game" (Base chain ref not applicable
 *               on Arbitrum).
 *         BullsBTC v1.02: ABI renames: wethFeed->wbtcFeed,
 *               setWethFeed->setWbtcFeed, pendingEthFeedChange->pendingBtcFeedChange,
 *               _readEthPrice->_readBtcPrice, ethReserveFeed->btcReserveFeed.
 *               NS-01: BTC/USD Arbitrum feed address marked UNVERIFIED.
 *               NS: INHERITED attribution fixed to BullsEth v2.26.
 *               NS: setWbtcFeed @param WETH->WBTC/USD. Brand history restored.
 *         BullsBTC v1.03: All BullsEth v2.27-v2.31 code fixes applied.
 *               D-1+v2.29: draw-30 holdback changed from MAX_TARGET_RETURN_BPS
 *               ceiling to 29-draw ogRatioBpsAccumulator running-average estimate.
 *               New constant MAX_TARGET_RETURN_BPS = 9000 added as fallback.
 *               v2.30: _finalizeWeekCore draw-30 exclusion guard added
 *               (currentDraw < TOTAL_DRAWS) -- true SSoT between holdback and
 *               closeGame(). Eliminates spurious EndgameShortfall in high-OG seasons.
 *               B-1: claimResetRefund() pool1 + pool2: p.totalPaid decremented
 *               by GROSS (grossEquiv) matching totalOGPrincipal basis.
 *               NS/IC: documentation sweeps from BullsEth v2.27-v2.31.
 *               See BullsEth_v2.31.sol for full analysis.
 *         Audit fixes from v1.1 review: HIGH-01, MEDIUM-01, MEDIUM-02, LOW-01/02/03 resolved.
 *         v1.3: Mulligan removed, grossEquiv restored.
 *         v1.4: NatSpec hygiene pass.
 *         v1.5: Dynamic starting breath (_computeStartingBreath). Starting breath calibrated to
 *               target T3 prizes near TICKET_PRICE at draw 1 (estimate, not a hard guarantee).
 *         v1.51: Safety pass -- entries guard, getRequiredCutoffDiffBounds(), endgame solvency check.
 *         v1.52: Aave removed entirely. Pure USDC contract. Gas reduced on all fund-movement paths.
 *         v1.53: NatSpec fixes.
 *         v1.54: Audit fixes M-01(NatSpec), M-02, M-03(NatSpec), L-01-05, I-03.
 *         v1.55: L-NEW-01(reverted in v1.56), L-NEW-02, I-NEW-01, I-NEW-02.
 *         v1.56: CRITICAL compile fix, MEDIUM filter align, LOW iter cap + SEQUENCER_FEED, INFO batch.
 *         v1.57-P1: Pregame fix. OG intent queue removed. OG registers immediately.
 *               cancelOGRegistration() replaces claimOGIntentRefund(). No ratio cap.
 *         v1.57-P2: Economics. UF_OG_TREASURY_BPS=1000 (10%). Graduated weekly treasury
 *               (500/1000/1500 bps). T3 graduated schedule (1/10 draw 1-2, 1/7 draw 5+).
 *               Return curve: 90% at <=20% OG, linear to 30% at 100% OG.
 *               Brand: NearestTheETH30 -> BullsEth. [BullsBTC forked from BullsEth v2.26.]
 *         v1.58-P3: Geometric breath engine. _solveGeometricBps() 24-iter binary search.
 *               OG obligation locked at startGame() (not draw 10). _simGeomPot solvency check.
 *         v1.59: Season-average OG ratio accumulator for closeGame() perOGPromised.
 *         v1.60: Pot-health-gated exhale floor. EXHALE_FLOOR_RELEASE_DEFAULT=12000 (120%).
 *               Immediate release when distressed. Governable [8000,20000] with 48h timelock.
 *         v1.61: Draw-7 breath bump eliminated (breathMultiplier write removed from
 *               _calibrateBreathTarget). Solver applies at draw 8 instead.
 *         v1.62: draw30BonusFund introduced. DRAW30_BONUS_BPS=300 (3% per draw siphoned
 *               draws 1-29, added to draw-30 surplus). Guarantees draw-30 is largest
 *               prize event. Dormancy sweep of bonus fund added. getSolvencyStatus
 *               and _computeBalanceAndAllocated include draw30BonusFund.
 *         v1.63: _computeStartingBreath denominator corrected for 87% distributable.
 *               grossEquiv uses draw-specific tRate. Guarantee language removed throughout.
 *               getProjectedEndgamePerOG cap corrected to targetReturnBps ceiling.
 *         v1.64-v1.66: NatSpec accuracy pass. Stale ~15% T3-winner references updated
 *               to graduated 18-22% schedule. DRAW PHASE FLOW diagram corrected.
 *         v1.67: Stale mulligan-used comment removed from _processMatchesCore.
 *         v1.68: currentDrawBonusContribution rollback on emergencyResetDraw. Fixes
 *               double-accumulation bug when draw is reset and replayed.
 *               BreathRecalibrated fourth field renamed newBreath -> computedBreath.
 *               potHealth cap removed from getProjectedEndgamePerOG.
 *         v1.69-v1.73: NatSpec passes. Carry-forward fixes. Stacked NatSpec split.
 *               Header changelog filled. @notice tags added. 0H/0M/0L/0NS.
 *         v1.74: withdrawTreasury and getSolvencyStatus @notice + @return tags.
 *         v1.75: Full @notice pass (46 functions). Header changelog completed.
 *               NS-M-01 _calibrateBreathTarget requiredEndPot attribution.
 *               NS-M-02 batchRefundPlayers phase. NS-M-03 closeGame callers.
 *               NS-L-01 through NS-L-05: orphaned @dev, duplicate @notice, stale labels.
 *               NS-I-01 through NS-I-04: dead code reason, @notice gaps, draw30BonusFund.
 *         v1.76: NS audit pass. 12 findings closed.
 *         v1.77: L-01 emergencyResetDraw governance completeness.
 *         v1.78: Feed events. Commitment credit rate fix. Zero-snapshot guard.
 *         v1.79: B-NEW-01 FeedSubstituted correct addresses. @return corrections.
 *         v1.80: B commitment credit rate. proposeDormancy feed cancel.
 *               @param sweep (18 functions). @return sweep (9 functions).
 *         v1.81: B-1.80-01/02 commitment credit dormancy fix. NS @param
 *               carry-forwards closed. pruneStaleOGs @notice. IC cleanup.
 *         v1.82: B-1.81-01 single-credit double-count fix. B-1.81-02 double-credit
 *               draw rate alignment. Both commitment credit paths use
 *               _getHistoricalTreasuryBps(currentDraw). emergencyResetDraw @notice.
 *         v1.83: tRate2 moved inside eligiblePool2 comment only -- declaration
 *               missing (compile error). Fixed in v1.84. @param min 1000->5000.
 *         v1.84: B-1.83-01 CRITICAL compile fix: tRate2 declaration was missing.
 *               NS-1.83-01: buyTickets @dev commitment credit rate updated.
 *         v1.85: NS-1.84-01 v1.83 changelog note. NS-1.84-02 @return example.
 *         v1.86: M-01 single-credit 2-ticket casualNetTicketTotal fix.
 *               NS-M-01 proposeBreathRails timelock 48h->7day. I-01 snapshot comment.
 *               NS-I-01 batchMarkLapsed @dev inline reimplementation note.
 *               NS-I-02 setReserveFeed/setWbtcFeed FeedSubstituted semantics comment.
 *         v1.87: NS-I-01 getContractVersion @return example. NS-I-02 v1.86 changelog.
 *         v1.88: B-1.87-01 action-2 guard removed (timing signal, graceful no-op).
 *               NS/IC sweep: sweep destinations, getOGListIndex, checkUpkeep action-2
 *               caveat, snapshotOGObligation draw-28/draw-7 distinction, _lockOGObligation
 *               retention reason, p.totalPaid magnitude, tRate pool1 inside branch.
 *         v1.89: NS-M-01 getAutoDefault @return cents. NS-L-01/02 version example
 *               and sweepResetRefundRemainder destination. NS-I-01/02 dev notes.
 *         v1.90: B-1.89-01 _computeStartingBreathFromTarget rail clamp unified.
 *               NS-L-01/02 withdrawTreasury lock + dead guard doc. NS-I-01/02.
 *               IC-L-01 draw-30 bonus comment. IC-I-01 OG_OBLIGATION_LOCK_DRAW.
 *         v1.91: NS-M-01 emergencyResetDraw permissionless continuation documented.
 *               NS-I-02 _computeStartingBreathFromTarget @dev. IC-L-01 rename
 *               usingDoubleCredit -> commitmentCreditHandled throughout buyTickets.
 *         v1.92: tRate2 tag [v1.83]->[v1.84]. claimDormancyRefund status-lost OG
 *               forfeiture documented as intentional. pruneStaleOGs @param. EMA
 *               comment scope corrected. registerAsWeeklyOG cap-exhaustion note.
 *         v1.93: countStaleOGs isWeeklyOG guard added. _upfrontOGCapReached dead-code
 *               NatSpec. _updateStreakTracking and _computeBalanceAndAllocated @dev.
 *               MatchCountMismatch @notice. 165 anchor comment. EMA OG floor note.
 *         v1.94: Duplicate @notice and @dev blocks removed (MatchCountMismatch,
 *               _computeStartingBreathFromTarget, _lockOGObligation). Paginated
 *               countStaleOGs invariant comment added.
 *         v1.95: IC-1.94-01 [v1.2 H-01] tierPools comment corrected (preserved
 *               not returned). IC-1.94-02 solver early-exit comment inverted logic.
 *         v1.96: NS-1.95-01 getContractVersion @return example removed permanently.
 *               NS-1.95-02 _lockOGObligation version reference updated.
 *         v1.97: All audit-firm name references removed from contract. Dead-code
 *               retention notes generalised to "audit trail continuity".
 *         v1.98: F1 MEDIUM: registerAsOG/cancelOGRegistration totalOGPrincipal uses
 *               OG_UPFRONT_COST (not ogTransfer) to match p.totalPaid for credit OGs.
 *               F4: claimDormancyRefund post-transfer solvency check (inline).
 *               [F2, F3: cover letter items, no code changes.]
 *               NS/IC sweep: N1/N2/N3/N8/N9/N11/N13, ogTransferAmount dead var removed.
 *         v1.99: B-1.98-01 CRITICAL compile fix: claimDormancyRefund F4 inlined
 *               (getSolvencyStatus is external, cannot be called internally).
 *               SYNC comments on getSolvencyStatus + _computeBalanceAndAllocated.
 *               NS/IC: OG_OBLIGATION_LOCK_DRAW warning, _solveGeometricBps insolvent
 *               scenario, _checkAutoAdjust EMA conservative clarification,
 *               claimDormancyRefund defensive guard, credit trade-off documentation.
 *         v2.00: Submission packaging pass. NS/IC verification sweep. No bytecode
 *               changes vs v1.99 beyond version string.
 *         v2.01: B-2.00-02: EMA moved above breathOverrideLock gate (revenue tracked
 *               during cooldown). B-2.00-01: claimDormancyRefund solvency check
 *               extended to include reset/commit pools. C-2.00-01: proposeBreathOverride
 *               80% gate documented. C-2.00-02 interaction cross-referenced.
 *               NS sweep: NS-2.00-01/02/03/04/05, C-2.00-03/04/05/06.
 *         v2.02: B-2.01-01: claimEndgame _allocated extended (reset/commit pools).
 *               B-2.01-02: constructor _validatePrediction for _defaultPrediction.
 *               [B-2.01-03: NOT APPLIED in v2.02 -- code change did not land.]
 *               C-2.01-06: claimPrize SolvencyAlert (incomplete set -- fixed v2.03).
 *               NS/IC sweep: NS-2.01-01/02/03/04/05/06/07/08, C-2.01-11.
 *         v2.03: B-2.01-03 APPLIED: isResultStale returns false during non-IDLE phases.
 *               B-2.02-02: claimPrize SolvencyAlert _allocated = full getSolvencyStatus set.
 *               NS-2.02-02/03: claimPrize SYNC comment, _continueUnwind streak note.
 *         v2.04: NS-2.03-01 _continueUnwind @dev corrected (removed inaccurate clause).
 *               NS-2.03-02 isResultStale @return documents phase exclusion.
 *               C-2.03-02 claimPrize tier loop style normalised.
 *         v2.05: M-01 closeGame surplusToTreasury emitted in GameClosed.
 *               M-02 claimDormancyRefund commitmentPaid gated on pool expiry.
 *               L-01 SolverDistressSignal event added (view regression fixed v2.06).
 *               L-02 TreasuryLocked uses requiredEndPot. L-03 pregameOGNetContributed
 *               includes commitment net (double-credit fix completed v2.06).
 *         v2.06: C-01 view removed from _solveGeometricBps (emit in view = compile error).
 *               L-01: commitNet uses creditAmount not TICKET_PRICE.
 *               NatSpec/IC sweep: withdrawTreasury @dev, duplicate @notice, broken sentence.
 *         v2.07: All audit firm name references stripped from contract and changelog.
 *               IC: commitNet attribution corrected to v2.05/v2.06.
 *               NS: @return added to _getDrawTreasuryBps/_getHistoricalTreasuryBps.
 *               NS: claimDormancyRefund @dev notes commitment gate behaviour.
 *         v2.08: NS duplicate @return removed (both treasury BPS helpers).
 *               NS @dev added to claimDormancyRefund NatSpec block.
 *               IC withdrawTreasury first comment corrected (requiredEndPot).
 *               IC predictionDraw zeroed in status-loss and pruneStaleOGs.
 *         v2.09: IC duplicate p.prediction2Draw = 0 removed (_processMatchesCore).
 *               NS stale cancelOGRegistration sentence removed (_cleanupOGOnRefund).
 *         v2.10: proposeBreathRails equal-rails guard strengthened (< → <=).
 *               IC _solveGeometricBps linear formula documented as _simGeomPot(0) equiv.
 *               NS getPlayerInfo @return tags added (14 values). OG_TREASURY_BPS note.
 *         v2.11: claimDormancyRefund trailing guard adds committedDoubleCount decrement.
 *               NS proposeBreathRails @param newMax strict > newMin documented.
 *         v2.12: IC claimDormancyRefund neverPlayedCommitmentCount dead-code commented.
 *         v2.13: B-L-01 SWEPT dead-code comment. NS-L-01 overflow exponent corrected.
 *               B-I-01/NS-L-02 getWinnerCounts IDLE behaviour. NS-I-01/02 warnings.
 *               IC-L-01 emergencyResetDraw version tag. IC-I-01 five SYNC locations.
 *         v2.14: IC-L-01 _finalizeWeekCore bonus clear comment corrected (not rollback).
 *               NS-I-01 OG_OBLIGATION_LOCK_DRAW duplicate comment removed.
 *               NS-I-02 KEEPER TRUST MODEL mismatch-loop documented.
 *
 *         v2.15: Flat treasury. LAUNCH_TREASURY_BPS_D1_2 / LAUNCH_TREASURY_BPS_D3_4
 *               deprecated (zero callers). _getDrawTreasuryBps() /
 *               _getHistoricalTreasuryBps() deprecated (zero callers -- both now
 *               return TREASURY_BPS for audit trail continuity).
 *               buyTickets(), claimDormancyRefund(), claimResetRefund() all use
 *               flat TREASURY_BPS (15%). Graduated rate asymmetry between pregame
 *               commitment (15%) and early-draw tickets (5%) removed.
 *               Reset pool sizing now exact -- no rate-delta trade-off.
 *               Trade-off documentation removed from buyTickets credit blocks.
 *               startGame() EMA seed comment corrected (flat rate, no split).
 *               NS: buyTickets @dev, claimDormancyRefund @dev, claimResetRefund
 *               @dev updated. IC: tRate/tRate2 local vars removed from
 *               claimResetRefund (inlined TREASURY_BPS).
 *               NOTE v2.15 audit trail: @return tags on both treasury BPS helpers
 *               (added v2.07, duplicate removed v2.08) were removed when v2.15
 *               simplified the functions to single-line stubs. No parameters
 *               remain to document. Closed in v2.16 header (NS-I-03).
 *
 *         v2.16: NS/IC pass (4 items).
 *               NS-I-01: _getDrawTreasuryBps() view -> pure. Returns only a
 *               constant; no state read. _getHistoricalTreasuryBps() already pure.
 *               NS-I-02: LAUNCH_TREASURY_BPS_D3_4 lacked its own @dev deprecation
 *               note. Solidity NatSpec attaches only to the immediately following
 *               declaration; D1_2's @dev did not cover D3_4. Added.
 *               NS-I-03: v2.07 header entry said "@return added to
 *               _getDrawTreasuryBps/_getHistoricalTreasuryBps". Those tags were
 *               lost when v2.15 simplified the functions to stubs. Note added to
 *               v2.15 entry closing the audit trail.
 *               IC-I-04: Section header "Helper functions for graduated economics"
 *               updated to "[v2.15] Deprecated treasury helpers" -- the section
 *               now contains only two stub functions, not graduated-rate logic.
 *
 *         v2.17: NS/IC pass (4 items).
 *               NS-L-01: withdrawTreasury() @dev first line said "at the graduated
 *               rate" -- stale since v2.15. Updated to flat TREASURY_BPS (15%).
 *               IC-L-01: claimDormancyRefund() commitment-path comment block
 *               referenced graduated rates that no longer exist. Replaced with
 *               clean flat-treasury statement. Deprecated function names removed
 *               from the inline comment.
 *               IC-I-01: _getHistoricalTreasuryBps() was positioned before the
 *               "[v2.15] Deprecated treasury helpers" section header; only
 *               _getDrawTreasuryBps() was inside it. Both deprecated stubs now
 *               colocated inside the section.
 *               IC-I-02: v2.16 header IC-I-04 entry said "Deprecated treasury
 *               helpers (v2.15)" (trailing parenthetical). Actual code reads
 *               "[v2.15] Deprecated treasury helpers" (leading bracket). Corrected
 *               in the v2.16 entry text above and in this header.
 *
 *         v2.18: T3 winner schedule redesigned. T1=1%, T2=5%, T3 graduated:
 *               draws 1-2: 6% (T3_WINNER_BPS_D1_2 1000->600),
 *               draw 3: 7% (T3_WINNER_BPS_D3 1111->700),
 *               draw 4: 8% (T3_WINNER_BPS_D4 1250->800),
 *               draw 5+: 9% (T3_WINNER_BPS 1429->900).
 *               Total winners: ~12% draws 1-2, ~15% draw 5+ (was ~18%/~22%).
 *               Pool splits unchanged (JP_BPS=4000, P2_BPS=3556). T2 winner
 *               percentage unchanged at 5% (was 7% in v1.57-P2 -- re-confirmed
 *               as T2=5% by design). Breath formula unchanged (reads
 *               _getT3WinnerBps() at runtime, self-calibrates automatically).
 *               Count bounds recalibrated: T2_COUNT_MIN_BPS 600->400 (cumulative
 *               T1+T2 target now 6%; old MIN=600 reflected prior T2=7% design giving
 *               8% cumulative; 400 maintains same 2pp margin below the 6% target);
 *               T3_COUNT_MIN_BPS 1600->1000 (cumulative target now 12% not 18%;
 *               same 2pp margin logic). T2_COUNT_MAX_BPS and T3_COUNT_MAX_BPS
 *               unchanged (same 2-ticket snapshot bias applies).
 *               ~28 NatSpec/IC locations updated to reflect new percentages.
 *
 *         v2.19: Triple audit pass on v2.18 changes. 0C/0H/0M/2L/4I. All resolved.
 *               NS-L-01: getRequiredCutoffDiffBounds @return t3Min/t3Max headline
 *               said "16%-50%" -- one missed location from v2.18 ~28 location sweep.
 *               Body NOTE correctly said "10% minimum." Direct contradiction.
 *               Fixed to "10%-50% [v2.18: was 16%]".
 *               NS-L-02: WORST-CASE SCENARIO #2 said "With T3=6% (fewer winners)"
 *               which implied a flat rate. T3 is graduated 6-9% by draw. Fixed to
 *               "T3=6-9% graduated schedule (6% draws 1-2, 9% draw 5+)".
 *               NS-I-01: Header v2.18 entry "cumulative T1+T2 target now 6% not 8%"
 *               -- "8%" unexplained. Added: prior v1.57-P2 T2=7% design gave 8%
 *               cumulative; 400 maintains same 2pp margin as old 600 did for 8%.
 *               NS-I-02: getRequiredCutoffDiffBounds "Verified safe at 20% OG +
 *               10% attrition" -- validated against old 16% MIN. Added v2.19 note:
 *               margin structurally identical (12%-10%=2pp = old 18%-16%=2pp).
 *               Re-confirm before production deployment.
 *               IC-L-01: _matchAndCategorize() had no cross-reference to draw-
 *               schedule-dependent T3 winner% via _getT3WinnerBps(). Added inline
 *               comment: 6% draws 1-2 / 7% draw 3 / 8% draw 4 / 9% draw 5+.
 *               IC-I-01: _calculatePrizePools() T3 pool comment noted size (21.3%)
 *               but not that winner count varies by draw. Added cross-reference
 *               to _getT3WinnerBps() and submitCutoffDiffs().
 *               IC-I-02: _computeStartingBreath() _getT3WinnerBps() call had no
 *               note that currentDraw=1 at startGame() makes it return 600 (draw-1
 *               rate). Added: tightest winner pool, intentional calibration target.
 *
 *         v2.20: NS pass (4 items). 0C/0H/0M/0L/3I from audit + 1 self-caught.
 *               All four are stale "~8%" T1+T2 cumulative references surviving
 *               from v1.57-P2 when T2 was 7% (T1+T2=8% cumulative). T2 is 5%
 *               since design redesign; correct cumulative T1+T2 is 6%.
 *               NS-I-01 (audit): Constants block cumulative comment t2Count "~8%" -> "~6%".
 *               NS-I-02 (audit): getCutoffState() @return _t2 "top 8%" -> "top ~6% cumulative".
 *               NS-I-03 (audit): submitCutoffDiffs() @param _t2CutoffDiff "~8%" -> "~6%".
 *               NS-I-04 (self-caught): submitCutoffDiffs() @dev COUNT SEMANTICS
 *               _t2Count said "~8%, T1+T2 combined" -- same stale figure in same
 *               function as NS-I-03, missed by audit. Fixed to "~6%".
 *
 *         v2.21: Full triple audit pass on v2.20. 0C/0H/0M/1L/3I. All resolved.
 *               NS-L-01: getRequiredCutoffDiffBounds() @return t2Min/t2Max said
 *               "6%-12%". T2_COUNT_MIN_BPS=400 (4%) since v2.18 -- direct counterpart
 *               to v2.19 NS-L-01 which fixed t3Min/t3Max. Fixed to "4%-12%".
 *               NS-I-01: _computeStartingBreath() overflow NatSpec coefficient
 *               was "7.86*10^22". Recalculated for T3_WINNER_BPS_D1_2=600: actual
 *               max = 10^7 * 110000 * 600 * 10^8 = 6.6*10^22. Exponent correct.
 *               NS-I-02: CutoffOutOfRange error @dev listed T1 and T3 count ranges
 *               but omitted T2 (4%-12%). Added t2Count line between T1 and T3.
 *               NS-I-03: _computeStartingBreath() TRADE-OFF closing "$175" figure
 *               was not marked superseded despite v2.18 opening the section with
 *               "[v2.18] Simulation values superseded". Tagged "[v2.18: superseded]".
 *
 *         v2.22: 1L/2I bugs + 3L/1I NS + 1L/2I IC. 14 changes.
 *               B-2.21-01 (L): isResultStale() draw-1 early return removed. Dead
 *               code -- superseded by general check. At draw 1 lastResolvedDraw==0:
 *               (0!=0)||(resolvedPrice==0) = resolvedPrice==0 = true.
 *               Draw-0 guard (currentDraw==0 return true) is intentionally retained:
 *               prevents uint256 underflow on currentDraw-1 in the general check.
 *               PREGAME always has currentDraw==0. Not superseded by the general check.
 *               B-2.21-02 (I): claimDormancyRefund() commitment path double-write
 *               of p.totalPaid=0 removed. Trailing cleanup at L2654 handles all paths.
 *               B-2.21-03 (I): getOGListIndex() NatSpec clarified -- returns 0 for
 *               both first entry and not-in-list. Usage pattern documented.
 *               NS-2.21-01 (L): claimPrize() @dev warning added: prizesSweepComplete
 *               permanently blocks all future claims. Frontend trap documented.
 *               NS-2.21-02 (L): getPreGameStats() readyToStart @return updated to
 *               include block.timestamp < signupDeadline + MAX_PREGAME_DURATION gate.
 *               NS-2.21-03 (L): Both deprecated treasury stub @dev comments updated:
 *               "post-audit-acceptance" -> "after Cyfrin audit acceptance and
 *               before mainnet deployment".
 *               NS-2.21-04 (I): SYNC @dev intro sentence added self-reference to
 *               _computeBalanceAndAllocated(). Intro listed 4 sites; count said 5.
 *               IC-2.21-01 (L): _computeTargetReturnBps() "Floor is unreachable"
 *               comment replaced. Distinguished dead ternary false branch from live
 *               3000 output reachable via true branch at 100% OG ratio.
 *               IC-2.21-02 (I): safeDouble_ min() guard comment added explaining
 *               invariant: guards counter drift, prevents underflow in singleCount_.
 *               IC-2.21-03 (I): draw30BonusFund "never resided in prizePot" comment
 *               corrected. USDC did flow through prizePot; accounting variable was
 *               isolated. "Never credited to prizePot accounting variable" is precise.
 *
 *         v2.23: 0C/0H/0M/3L/4I. All NS/IC only. Zero bytecode changes.
 *               B-L-01: isResultStale() draw-0 guard lacked comment marking it as
 *               intentional. v2.22 B-2.21-01 rationale ("superseded by general check")
 *               could mislead a maintainer into removing the draw-0 guard, which prevents
 *               uint256 underflow on currentDraw-1. Comment + header note added.
 *               NS-L-01: getSolvencyStatus() @return isSolvent said "totalValue >=
 *               totalAllocated". Actual code uses SOLVENCY_TOLERANCE (100_000 = $0.10).
 *               Fixed: "True if totalValue + SOLVENCY_TOLERANCE >= totalAllocated."
 *               NS-L-02: isResultStale() @return silent on currentDraw==0 (PREGAME).
 *               PREGAME returns true from draw-0 guard before general check runs --
 *               contradicts "False during any non-IDLE phase". Sentence added.
 *               NS-I-01: _lockOGObligation() said "external audit acceptance".
 *               Standardised to match NS-2.21-03 language: "Cyfrin audit acceptance
 *               and before mainnet deployment".
 *               NS-I-02: sweepUnclaimedEndgame() and sweepUnclaimedPrizes() had no
 *               @dev. Both now document the 180-day timing gate, settlementTimestamp
 *               origin, and permanence of the sweep for individual claimants.
 *               IC-L-01: _finalizeWeekCore() scheduleAnchor re-anchor formula had no
 *               comment. scheduleAnchor=block.timestamp-currentDraw*DRAW_COOLDOWN;
 *               lastDrawTimestamp=scheduleAnchor+currentDraw*DRAW_COOLDOWN is
 *               effectively lastDrawTimestamp=block.timestamp. Purpose: re-anchor
 *               schedule so next draw cooldown starts from reset-finalize time.
 *               IC-I-01: _calculatePrizePools() _checkAutoAdjust() called on
 *               post-weeklyPool-deduction prizePot with no comment. Added: solver
 *               intentionally sees reduced pot (plans remaining draws from new base).
 *               IC-I-02: _checkAutoAdjust() EMA "3:1 weighted blend" did not state
 *               which side carries the 3. Added: "3 parts historical, 1 part current
 *               draw. ~5 draws to halve if revenue drops to zero." [CORRECTED v2.24:
 *               "~5 draws" was wrong; correct is ~2-3 draws (ln(0.5)/ln(0.75)=2.4)].
 *               IC-L-01: see below.
 *
 *         v2.24: 0C/0H/0M/3L/3I. 1 code fix + 2L/3I NS/IC.
 *               B-L-01: cancelOGRegistration() deficit path: when treasuryBalance <
 *               deficit, treasuryBalance was not decremented. Transfer still executed,
 *               leaving treasuryBalance overstated (phantom balance). Fixed: else branch
 *               zeroes treasuryBalance entirely. SafeERC20 backstops actual USDC.
 *               IC-I-01: Companion comment for B-L-01 -- documents the drain-to-zero
 *               behaviour and SafeERC20 revert guarantee.
 *               NS-L-01: sweepResetRefundRemainder() @notice said "reset refund pools"
 *               only -- omits commitmentRefundPool. Material to users who have commitment
 *               refund entitlements. @notice expanded to name all three pools.
 *               IC-L-01: _checkAutoAdjust() EMA halving time "~5 draws" was a factual
 *               error introduced in v2.23 IC-I-02. Correct: 0.75^n=0.5 gives
 *               n=ln(0.5)/ln(0.75)=2.4 draws. Fixed to "~2-3 draws". Also fixed
 *               "Slow decay" -> "Fast decay" (correct characterisation at 3:1 ratio)
 *               and updated the same "~5 draws" mention in the v2.23 header entry.
 *               NS-I-01: getDraw30BonusStatus() @return perDrawEstimate noted only
 *               breathMultiplier dependency. Actual calculation uses getCurrentPrizeRate()
 *               which incorporates prizeRateMultiplier too. @return updated.
 *               NS-I-02: proposeBreathRails() @param newMax "Equal rails bypass the
 *               geometric solver" read as a feature description of a permitted input.
 *               Equal rails REVERT. Rewrote as explicit prohibition with redirect.
 *
 *         v2.25: 0C/0H/0M/2L/3I. NS/IC only. Zero bytecode changes.
 *               NS-L-01: withdrawTreasury() @dev said "Treasury accrues from weekly
 *               ticket sales at flat 15%". Treasury accrues from 5 paths at 2 rates
 *               (15% and 10%). Updated to bullet-list all sources and rates.
 *               NS-L-02: sweepFailedPregame() @notice said "Returns all OG
 *               registrations and commitment payments" -- implies individual player
 *               refunds. Function sweeps residual balance to PROTOCOL_BENEFICIARY.
 *               Rewrote to describe actual behaviour.
 *               NS-I-01: MalformedPerformData error was undocumented unused code.
 *               All other deprecated/unused items carry annotations. Added @dev
 *               explaining retention for ABI compatibility and original intent.
 *               IC-L-01: _calculatePrizePools() draw-30 comment did not mention
 *               that draw30BonusFund is already net of treasury. Added NOTE.
 *               IC-I-01: performUpkeep() action 2 decode block had no comment
 *               explaining why MalformedPerformData is not used here. Added
 *               cross-reference to error declaration.
 *               IC-I-02: _checkAutoAdjust() EMA seed note said "casual-only" without
 *               mentioning committedDoubleCount. Updated for precision; rest of
 *               block preserved (OG-heavy inflation note, convergence time).
 *
 *         v2.26: 0C/0H/0M/0L/4I. NS/IC only. Zero bytecode changes.
 *               NS-I-01: _continueUnwind() @dev streak sentence said "Players below
 *               WEEKLY_OG_QUALIFICATION_WEEKS reset to 1" -- factually wrong. Code
 *               does not touch consecutiveWeeks at all. Reset happens in
 *               _updateStreakTracking() on next buyTickets() if gap detected, applying
 *               to all players equally. Corrected.
 *               NS-I-02: getExhaleFloorHealth() @return potHealthBps: sentinel
 *               potHealthBps=10000 when requiredEndPot==0 (no OG obligation) was
 *               undocumented. Monitoring tools see "100% health" with no context.
 *               IC-I-01: _continueUnwind() qualifiedWeeklyOGCount++ lacked context.
 *               Increment is a symmetric restoration of the decrement in
 *               _processMatchesCore() at status loss. Cross-reference added.
 *               IC-I-02: sweepFailedPregame() gate conditions lacked semantic
 *               comments. Two distinct paths: time-gate (extension elapsed, residuals
 *               sweep regardless) and clean-close (all players refunded). Named.
 *
 *         KEEPER TRUST MODEL:
 *         submitCutoffDiffs() is callable by owner or automationForwarder only.
 *         Same trust level as resolveWeek(), emergencyResetDraw(), and all existing
 *         keeper operations. A compromised keeper can submit diffs that misplace tier
 *         boundaries -- this is mitigated by THREE layers:
 *           Layer 1: Count range verification in submitCutoffDiffs() (T1: 0.5-4%,
 *                    T2: 4-12%, T3: 10-50% of snapshotTotalEntries).
 *                    (T3 upper bound wide due to 2-ticket casual snapshot bias -- see M-02).
 *           Layer 2: Post-match reconciliation in _processMatchesCore() -- actual
 *                    winner array sizes re-verified against same bounds. Mismatch
 *                    emits MatchCountMismatch and reverts to CUTOFF_SUBMISSION.
 *           Layer 3: getRequiredCutoffDiffBounds() view -- keepers call this before
 *                    submitting to pre-validate their computed counts on-chain.
 *         No keeper action can steal funds. At worst a faulty keeper delays a draw
 *         by up to DRAW_STUCK_TIMEOUT (48h) before emergencyResetDraw() is available.
 *         MISMATCH LOOP: A malicious keeper can reset the 48h window by submitting
 *         count-valid but diff-invalid cutoffs (MatchCountMismatch restarts
 *         phaseStartTimestamp). Mitigation: setAutomationForwarder(address(0))
 *         revokes keeper access; owner submits correct cutoffs directly.
 *         Monitor for repeated MatchCountMismatch events on the same draw.
 *
 *         WORST-CASE SCENARIOS (documented for deployment runbook):
 *           1. Keeper offline: draw stuck in CUTOFF_SUBMISSION for 48h, then
 *              owner calls emergencyResetDraw(). Players refunded from resetDrawRefundPool.
 *           2. 3% OG, all 2-ticket casuals: Starting breath calibrated to target T3 near
 *              TICKET_PRICE ($10). With T3=6-9% graduated schedule (6% draws 1-2,
 *              9% draw 5+), the rail is less likely to bind vs old 10-14.3% design.
 *              Calibration approximation -- actual prize depends on runtime parameters.
 *              [v2.18] $9.45 figure superseded.
 *           3. Weekly OG misses buy: status lost immediately (no mulligan), endgame
 *              not qualified. Intentional design.
 *           4. (No Aave in v1.52 -- Aave liquidity crunch scenario not applicable.)
 *           5. [v1.03] Emergency reset on draw 30: draw voided, casuals refunded,
 *              OG status restored. TOTAL_DRAWS guard closes game WITHOUT draw-30
 *              prize distribution. Season ends on refunds, not replay. No fund risk.
 *
 *         CORE MECHANIC CHANGE FROM 1Y GAME:
 *         Fixed BPS bands replaced with dynamic percentage-based tiers.
 *         After price resolves, keeper submits cutoff diffs defining the top 1% (T1)
 *         and top ~12-15% (T1+T2+T3, draw-schedule dependent) by proximity.
 *         Contract verifies then distributes.
 *         T1 (1% Club) always has winners. No jackpot miss possible. Seed (10%)
 *         returns to prizePot each draw as rollover -- pot grows draw-on-draw.
 *
 *         TIER STRUCTURE (% of entries):
 *           T1 (1% Club): top 1% -- share jackpot pool (40% of distributable)
 *           T2:           next 5% -- share T2 pool (35.56% of distributable)
 *           T3:           next 6-9% (draw-schedule: 6% draws 1-2, 7% draw 3,
 *                         8% draw 4, 9% draw 5+) -- share T3 pool (remainder)
 *           Seed/rollover: 10% of weeklyPool -- returns to prizePot every draw
 *           Buffer: tie clusters at boundaries all included (~2% tolerance)
 *         Buying 2 tickets = 2 independent entries = double chance of 1% Club.
 *
 *         DRAW PHASE FLOW:
 *           IDLE -> CUTOFF_SUBMISSION -> MATCHING -> DISTRIBUTING -> FINALIZING
 *         resolveWeek() transitions IDLE -> CUTOFF_SUBMISSION. Keeper computes cutoff
 *         diffs off-chain, calls submitCutoffDiffs(), contract verifies, transitions
 *         CUTOFF_SUBMISSION -> MATCHING. If cutoffs not submitted within DRAW_STUCK_TIMEOUT (48h),
 *         emergencyResetDraw() available.
 *
 *         KEY PARAMETER CHANGES FROM 1Y GAME:
 *         TOTAL_DRAWS: 52->30 | DRAW_COOLDOWN: 7days->72h | PICK_DEADLINE: 4days->48h
 *         OG_UPFRONT_COST: $1040->$600 (2x total season cost, 60x single ticket price)
 *         INHALE_DRAWS: 36->20 | FINAL_CALIBRATION_DRAW: 48->28
 *         POST_LOCK_DRAWS: 42->20 | WEEKLY_OG_QUALIFICATION_WEEKS: 51->30
 *         MULLIGAN_THRESHOLD: removed entirely (no mulligan in 90-day game) | ENDGAME_SWEEP_WINDOW: 365->180 days
 *         RESET_REFUND_WINDOW: 60->30 days | JP_BPS/P2_BPS recalibrated for 3 tiers
 *         TIER1-4_BPS, BAND_DENOMINATOR, JP_MISS_*_BPS: removed entirely
 *         tier1-4Band state vars removed | p4Winners removed | tierPools[4]->tierPools[3]
 *         InvalidTierConfiguration guard removed (no TIER_BPS constants)
 *         getWinnerCounts() returns 3 values not 4 (ABI CHANGE -- update subgraphs)
 *         JPMissRedistributed event removed | TierNoWinners event removed
 *
 *         ALL OG, BREATH, DORMANCY, AND EMERGENCY RESET MECHANICS
 *         INHERITED UNCHANGED FROM BullsEth v2.26 (which itself forked NearestTheETH_Base_1Y v1.86).
 *         (Aave mechanics removed in v1.52 -- pure USDC contract.)
 *
 *         Network: Arbitrum One
 *         BTC/USD feed: *** UNVERIFIED -- CONFIRM BEFORE DEPLOYMENT ***
 *                       Candidate A: 0x6ce185539ad4fdaecd227730de3c08fc5b1a08e9
 *                       Candidate B: 0x6ce185860a4963106506C203335A2910413708e9
 *                       Both match 0x6ce1...08e9 pattern (Chainlink data page).
 *                       Verify at docs.chain.link/data-feeds/price-feeds/addresses
 *                       (Arbitrum One, 8 dec, ~0.05% deviation).
 *         USDC: 0xaf88d065e77c8cC2239327C5EDb3A432268e5831 (native Circle USDC on Arbitrum)
 *         Sequencer: 0xFdB631F5EE196F0ed6FAa767959853A9F217697D (Arbitrum Mainnet)
 *         (No Aave dependency -- pure USDC contract.)
...
contract BullsBTC is ReentrancyGuard, Ownable2Step {
    ...
    ///      before this timestamp for a 90% refund.
    ...
    ///      netRefund = ogTransfer * 90%. Treasury slice (10%) is permanently retained.
    ...
    ///      requiredPot = obligation * targetReturnBps/10000 + DRAW30_PRIZE_RESERVE.
    ...
    // this 30-draw game (no exhale premium). Dead branch removed from buyTickets().
    uint256 public constant OG_UPFRONT_COST                = 600_000_000; // $600 = 30 draws x $10 x 2 tickets (2x total season cost, 60x single ticket price)
    // [v1.57-P2] OG_TREASURY_BPS deprecated. Upfront OGs use UF_OG_TREASURY_BPS (1000).
    //              Weekly OGs use TREASURY_BPS (1500). This constant is now unreferenced.
    //              Retained for ABI compatibility. Numeric value (1500) coincidentally matches
    //              TREASURY_BPS but the two were semantically distinct: this was the OG
    //              treasury rate before v1.57-P2 changed it to UF_OG_TREASURY_BPS (10%).
    ...
    ///      in BullsBTC. The qualification threshold is only reachable on the final draw.
    ...
    // [v1.3] MULLIGAN_THRESHOLD removed. No mulligan in BullsBTC 30-draw game.
    ...
    /// @dev Upfront OGs pay 10% treasury (committed capital discount vs 15% casual rate).
    uint256 public constant UF_OG_TREASURY_BPS          = 1000;
    /// @dev [DEPRECATED v2.15] Graduated treasury removed. Zero callers.
    ///      Retained for ABI compatibility. See TREASURY_BPS (1500) for the flat rate.
    ...
    ///      within this period for 90% refund (10% treasury slice kept). After expiry: permanent.
    ...
    ///      when targetReturnBps = 9000 (90% -- low OG concentration).
    ...
    uint256 public constant TARGET_RETURN_FLOOR_BPS = 3000;  // informational only -- _computeTargetReturnBps() uses literal 3000 directly
    /// @dev [v2.27/v1.03] Ceiling of _computeTargetReturnBps() curve. Used for
    ///      draw-30 holdback fallback (ogRatioDrawCount==0 guard). Primary holdback
    ///      uses the 29-draw running average (v2.29/v2.30). [D-2 RESOLVED v2.30]
    ///      See BullsEth_v2.31.sol for full SSoT analysis.
    uint256 public constant MAX_TARGET_RETURN_BPS    = 9000;  // must match _computeTargetReturnBps() ceiling
    ...
    PendingFeedChange public pendingBtcFeedChange;
    ...
    ///      (commitment credit is taxed at 15%, OG transfer at 10%).
    ...
    ///      across all non-reset draws EXCEPT draw 30 (v1.03/v2.30 SSoT guard).
    uint256 public ogRatioBpsAccumulator;
    /// @dev [v1.59] Count of draws in ogRatioBpsAccumulator.
    ///      [v1.03/v2.30] Draw 30 excluded by SSoT guard in _finalizeWeekCore().
    ...
    ///          counted twice in ogList + once in weeklyNonOGPlayers = 3x snapshot, 1-2 actual.
    ///          Same overcount direction as (2). MAX bounds absorb.
    ...
    /// @notice Deploys BullsBTC_NoAave. Pure USDC contract -- no Aave dependency.
    /// @param _usdc                  USDC token address. Arb: 0xaf88d065e77c8cC2239327C5EDb3A432268e5831
    /// @param _btcFeed               Chainlink BTC/USD feed (8 decimals).
    ///                               [UNVERIFIED] Candidate A: 0x6ce185539ad4fdaecd227730de3c08fc5b1a08e9
    ///                               Candidate B: 0x6ce185860a4963106506C203335A2910413708e9
    ///                               Confirm at docs.chain.link before deployment.
    /// @param _defaultPrediction     Default prediction in USD cents for draw-1 auto-default.
    /// @param _sequencerFeed         L2 sequencer uptime feed. Arb: 0xFdB631F5EE196F0ed6FAa767959853A9F217697D
    /// @param _protocolBeneficiary   Immutable recipient for all post-game unclaimed sweeps.
    constructor(
        address _usdc, address _btcFeed,
        uint256 _defaultPrediction, address _sequencerFeed, address _protocolBeneficiary
    ) Ownable2Step() {
        if (_usdc == address(0)) revert InvalidAddress();
        if (_btcFeed == address(0)) revert InvalidAddress();
        ...
        try AggregatorV3Interface(_btcFeed).decimals() returns (uint8 dec) {
    ...
    /// @param prediction  BTC/USD price prediction in USD cents.
    ...
    /// @param prediction   First BTC/USD prediction (USD cents).
    /// @param prediction2  Second BTC/USD prediction (USD cents).
    ...
    ///         cancellation -- only the OG transfer net of 10% is returned.
    ///         A 72-hour voluntary decline window opens. Call cancelOGRegistration() within
    ///         72 hours for a 90% refund (10% treasury slice non-refundable as commitment).
    ...
    /// @param prediction   BTC/USD price in USD cents. Primary prediction for all draws.
    ...
    function registerAsOG(uint256 prediction, uint256 prediction2) external nonReentrant {
        ...
        // [v1.57-P2] UF OGs pay reduced treasury rate (10% not 15%) -- committed capital discount
    ...
    ///         Returns 90% of the OG transfer (ogTransfer * 90%).
    ///         The 10% treasury slice is non-refundable -- it was the commitment signal.
    ...
    ///         call registerAsOG() again (paying another 10% treasury slice each time).
    ...
    ///      (commitment paid 15% treasury; OG transfer paid 10% UF treasury).
    function cancelOGRegistration() external nonReentrant {
        ...
        // commitment credit was used (commitment taxed at 15%, OG at 10%).
        // ogCancelRefund = ogTransfer × 90%. Commitment credit is forfeited on cancel.
    ...
    ///         Missing a draw loses status -- no mulligan in BullsBTC.
    ...
    /// @param prediction   First BTC/USD prediction for the current draw (USD cents).
    /// @param prediction2  Second BTC/USD prediction for the current draw (USD cents).
    ...
    /// @notice Sets the reserve BTC/USD price feed used as primary fallback. Owner only.
    ...
    function setReserveFeed(address _reserveFeed) external onlyOwner {
        ...
        if (_reserveFeed == address(0)) { address old = btcReserveFeed; btcReserveFeed = address(0); emit FeedSubstituted(old, address(0)); return; }
        ...
        if (_reserveFeed == btcFeed) revert FeedUnchanged();
        if (_reserveFeed == wbtcFeed) revert FeedUnchanged();
        ...
        address oldReserve = btcReserveFeed;
        btcReserveFeed = _reserveFeed;
        emit FeedSubstituted(oldReserve, _reserveFeed);
    }

    /// @notice Sets the WBTC/USD price feed used as secondary fallback. Owner only.
    /// @param _wbtcFeed  New WBTC/USD fallback feed address. address(0) clears the fallback.
    function setWbtcFeed(address _wbtcFeed) external onlyOwner {
        // FeedSubstituted reused for admin feed management -- semantically distinct from startGame() usage.
        if (_wbtcFeed == address(0)) { address old = wbtcFeed; wbtcFeed = address(0); emit FeedSubstituted(old, address(0)); return; }
        if (_wbtcFeed == USDC || _wbtcFeed == address(this)) revert InvalidAddress();
        if (_wbtcFeed == SEQUENCER_FEED && SEQUENCER_FEED != address(0)) revert InvalidAddress();
        if (_wbtcFeed == btcFeed) revert FeedUnchanged();
        if (_wbtcFeed == btcReserveFeed) revert FeedUnchanged();
        try AggregatorV3Interface(_wbtcFeed).decimals() returns (uint8 dec) { if (dec != 8) revert FeedDecimalsMismatch(); } catch { revert FeedDecimalsMismatch(); }
        address oldWeth = wbtcFeed;
        wbtcFeed = _wbtcFeed;
        emit FeedSubstituted(oldWeth, _wbtcFeed);
    }
    ...
    /// @dev [v1.57-P2] STEP 1 computes targetReturnBps from actual OG ratio (90% at <=20% OG,
    ///      linear to 30% at 100% OG). STEP 2 derives ogBreath from targetReturnBps.
    ...
    function startGame() external onlyOwner nonReentrant {
        ...
        int256 price = _readBtcPrice();
        if (price == 0 && btcReserveFeed != address(0)) {
            int256 rPrice = _readPriceFeed(btcReserveFeed);
            if (rPrice > 0) {
                bool decimalOk;
                try AggregatorV3Interface(btcReserveFeed).decimals() returns (uint8 dec) { decimalOk = (dec == 8); } catch { decimalOk = false; }
                if (decimalOk) { emit FeedSubstituted(btcFeed, btcReserveFeed); btcFeed = btcReserveFeed; price = rPrice; }
            }
        }
        ...
        // obligation. It can only decrease as OGs lose status. No draw-10 lock needed.
        {
            uint256 maxOGs = upfrontOGCount + earnedOGCount;
            ogEndgameObligation = maxOGs * OG_UPFRONT_COST;
            // requiredEndPot = OG obligation at targetReturnBps + draw-30 prize planning floor.
            requiredEndPot = ogEndgameObligation * targetReturnBps / 10000 + DRAW30_PRIZE_RESERVE;
            ...
            // Uses flat TREASURY_BPS=15% for all draws -- seed is accurate.
    ...
    function claimSignupRefund() external nonReentrant {
        ...
        // [v1.57-P1] ogIntentStatus never written in v1.57+ -- SWEPT block unreachable.
        // If somehow reachable alongside isUpfrontOG path, _cleanupOGOnRefund above
        // would already have decremented committedPlayerCount (double-decrement trap).
        if (ogIntentStatus[msg.sender] == OGIntentStatus.SWEPT) {
            ogIntentAmount[msg.sender] = 0; ogIntentStatus[msg.sender] = OGIntentStatus.DECLINED;
            if (committedPlayerCount > 0) committedPlayerCount--;
        }
    ...
    /// @notice Removes stale weekly OGs from ogList. Callable by owner or automationForwarder.
    ...
    function pruneStaleOGs(uint256 maxPrune) external {
        if (msg.sender != owner() && msg.sender != automationForwarder) revert OwnableUnauthorizedAccount(msg.sender);
        if (drawPhase != DrawPhase.IDLE) revert DrawInProgress();
    ...
    /// @dev [v2.15] Treasury rate is flat TREASURY_BPS (15%) on all draws.
    ...
    function buyTickets(uint256 ticketCount) external nonReentrant {
        ...
        cost = TICKET_PRICE * ticketCount; // [v1.54] L-04: exhale pricing removed (equal to TICKET_PRICE in this 30-draw game)
        ...
        if (usingCommitment) {
            ...
            if (p.commitmentDouble && ticketCount == 2) {
                p.commitmentDouble = false;
                if (committedDoubleCount > 0) committedDoubleCount--;
                // [v2.15] Flat rate: pregame commitment (15%) and draw tickets (15%) match exactly.
                ...
                // [v2.15] Flat rate: TREASURY_BPS (15%) on both pregame and active draws.
        ...
        if (isActiveWeeklyOG) { totalOGPrincipal += cost; }
    ...
    /// @notice Submits or updates the second prediction for the current draw (OGs only).
    /// @param prediction2  Second BTC/USD price prediction in USD cents.
    ...
    /// @notice Resolves BTC price and transitions to CUTOFF_SUBMISSION.
    ...
    function resolveWeek() external nonReentrant {
        ...
        int256 currentPrice = _readBtcPrice();
        if (currentPrice > 0) { lastValidPrice = currentPrice; }
        else {
            if (btcReserveFeed != address(0)) {
                int256 rPrice = _readPriceFeed(btcReserveFeed);
                if (rPrice > 0) { currentPrice = rPrice; lastValidPrice = rPrice; emit ReserveFeedUsed(btcReserveFeed, currentDraw); }
            }
            if (currentPrice == 0 && wbtcFeed != address(0)) {
                int256 wPrice = _readPriceFeed(wbtcFeed);
                if (wPrice > 0) { currentPrice = wPrice; lastValidPrice = wPrice; emit ReserveFeedUsed(wbtcFeed, currentDraw); }
            }
    ...
    /// @dev    [v1.1] Called in CUTOFF_SUBMISSION phase by owner or automationForwarder.
        ...
        if (ogMatchingDone) {
            ...
            while (matchNonOGIndex < nonOGTotal && processed < MAX_MATCH_PER_TX) {
                ...
                if (p.lastTicketCount >= 2 && p.prediction2Draw == currentDraw && p.prediction2 != 0) {
                    _matchAndCategorize(addr, p.prediction2);
                }
    ...
    function _finalizeWeekCore() internal {
        ...
        if (isResetFinalize) { scheduleAnchor = block.timestamp - currentDraw * DRAW_COOLDOWN; }
        ...
        // [v1.03/v2.30] currentDraw < TOTAL_DRAWS: excludes draw 30 from accumulator
        // so ogRatioDrawCount=29 at closeGame(), matching the draw-30 holdback. True SSoT.
    ...
    ///         Callable by owner or automationForwarder.
    ...
    ///      Reset-finalize draws and draw 30 excluded (v1.03/v2.30 SSoT guard) -- only draws 1-29.
    ...
    function closeGame() external nonReentrant {
        ...
        // [v1.03/v2.30] Single-source-of-truth: ogRatioBpsAccumulator / ogRatioDrawCount
        // is the same 29-draw figure used for the draw-30 holdback. ogRatioDrawCount=29
        // at closeGame() because _finalizeWeekCore() excludes draw 30 (currentDraw < TOTAL_DRAWS guard).
        // OG ratio monotonically non-increasing (frozen denominator). No late-game spike possible.
        ...
        uint256 avgRatioBps = (ogRatioDrawCount > 0)
            ...
            : (ogCapDenominator > 0
                ...
                : 0); // fallback: should never fire -- ogRatioDrawCount=30 at closeGame
    ...
    ///         return; may be reduced on shortfall (EndgameShortfall event).
    ...
    /// @notice Proposes emergency dormancy activation with 48h timelock. Owner only.
    function proposeDormancy() external onlyOwner {
        ...
        if (pendingBtcFeedChange.effectiveTime != 0) { delete pendingBtcFeedChange; emit FeedChangeCancelled(); }
    ...
    /// @notice Executes dormancy after the 48h timelock. Distributes all funds. Owner only.
    function activateDormancy() external onlyOwner nonReentrant {
        ...
        if (ogPrincipal == 0 || (upfrontOGCount == 0 && weeklyOGCount == 0)) {
            ...
            dormancyOGPool = ogPrincipal; dormancyOGPoolSnapshot = ogPrincipal; dormancyPrincipalFullCover = true; prizePot -= ogPrincipal;
        } else {
        ...
        dormancyParticipantCount = upfrontOGCount + weeklyOGCount + weeklyNonOGPlayers.length;
    ...
    /// @dev [v2.15] Casual ticket refund uses flat TREASURY_BPS (15%) on all draws.
    ...
    function claimDormancyRefund() external nonReentrant {
        ...
        if (p.isUpfrontOG) {
            ...
            uint256 principal;
            if (dormancyPrincipalFullCover) { principal = p.totalPaid; }
            else { if (totalOGPrincipalSnapshot == 0) revert NothingToClaim(); principal = p.totalPaid * dormancyOGPoolSnapshot / totalOGPrincipalSnapshot; }
            if (principal > dormancyOGPool) principal = dormancyOGPool;
            dormancyOGPool -= principal; refund = principal;
            ...
            // [v2.15] Flat treasury: all draws use TREASURY_BPS (15%).
            ...
            // [v2.15] Flat treasury: pregame commitment paid at TREASURY_BPS (15%) pre-game.
    ...
    function batchRefundPlayers(address[] calldata playerList) external onlyOwner nonReentrant {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            // [v1.57-P1] Same SWEPT dead-code trap as claimSignupRefund -- unreachable.
            if (ogIntentStatus[addr] == OGIntentStatus.SWEPT) {
                ogIntentAmount[addr] = 0; ogIntentStatus[addr] = OGIntentStatus.DECLINED;
                if (committedPlayerCount > 0) committedPlayerCount--;
            }
            if (refund <= prizePot) { prizePot -= refund; } else {
                uint256 fromTreasury = refund - prizePot; prizePot = 0;
    ...
    /// @dev [v2.15] Flat treasury: both pools use TREASURY_BPS (15%) regardless of draw.
    ...
    function claimResetRefund() external nonReentrant {
        ...
        if (eligiblePool1) {
            ...
            if (claim > 0) {
                ...
                if (p.isWeeklyOG) {
                    // [v1.03/v2.27] B-1: grossEquiv computed before totalPaid decrement.
        ...
        if (eligiblePool2) {
            ...
            if (p.isWeeklyOG) {
                // [v1.03/v2.27] B-1: grossEquiv2 computed before totalPaid decrement (pool2).
    ...
    function claimCommitmentRefund() external nonReentrant {
        ...
        // [v1.57-P2] Commitment was paid pre-game at TREASURY_BPS (15%). No graduated rate.
    ...
    function claimPrize() external nonReentrant {
        ...
        { uint256 _b = IERC20(USDC).balanceOf(address(this));
          ...
          uint256 _a = prizePot + totalUnclaimedPrizes + treasuryBalance + endgameOwed
              ...
              + dormancyPerHeadPool + _tierTotal + currentDrawSeedReturn
    ...
    /// @dev [v1.0] Treasury accrues from multiple sources at two rates:
    ///      - Active-draw ticket sales (buyTickets): flat TREASURY_BPS (15%).
    ///      - Pregame commitments (payCommitment/payCommitmentDouble): TREASURY_BPS (15%).
    ///      - Weekly OG registration transfer (registerAsWeeklyOG): TREASURY_BPS (15%).
    ///      - Upfront OG registration transfer (registerAsOG): UF_OG_TREASURY_BPS (10%).
    ///      - OG cancellation retained slice (cancelOGRegistration): 10% of ogTransfer.
    ...
    ///      requiredEndPot is the geometric solver OG floor (ogEndgameObligation * targetReturnBps / 10000).
    ...
    /// @param amount     Amount of USDC (6-dec) to withdraw.
    ...
    /// @param newFeed  Address of the new primary BTC/USD Chainlink feed (8 decimals required).
    function proposeFeedChange(address newFeed) external onlyOwner {
        ...
        if (newFeed == btcFeed) revert FeedUnchanged();
        if (newFeed == btcReserveFeed) revert FeedUnchanged();
        if (newFeed == wbtcFeed) revert FeedUnchanged();
        if (pendingBtcFeedChange.effectiveTime != 0) revert TimelockPending();
        ...
        pendingBtcFeedChange = PendingFeedChange(newFeed, block.timestamp + TIMELOCK_DELAY);
    ...
    function executeFeedChange() external onlyOwner {
        if (gamePhase != GamePhase.ACTIVE) revert WrongPhase();
        if (drawPhase != DrawPhase.IDLE) revert DrawInProgress();
        if (pendingBtcFeedChange.effectiveTime == 0) revert NoTimelockPending();
        if (block.timestamp < pendingBtcFeedChange.effectiveTime) revert TooEarly();
        address oldFeed = btcFeed;
        btcFeed = pendingBtcFeedChange.newFeed;
        lastValidPrice = 0; autoDefaultCents = 0;
        emit FeedChangeExecuted(oldFeed, btcFeed);
        delete pendingBtcFeedChange;
    }
    ...
    /// @notice Initiates an emergency draw reset. Rolls back current draw for replay.
    ...
    function _continueUnwind() internal {
        ...
        for (uint256 i = start; i < end; i++) {
            ...
            // [v1.3] No mulligan unwind needed -- mulligan removed from BullsBTC.
        }
    ...
    function getSolvencyStatus() external view returns (uint256 totalValue, uint256 totalAllocated, bool isSolvent) {
        totalValue = IERC20(USDC).balanceOf(address(this)); // [v1.52] USDC only, no aUSDC
        uint256 tierPoolsTotal;
        // [v1.0] 3 tiers only.
        for (uint256 i = 0; i < 3; i++) tierPoolsTotal += tierPools[i];
        totalAllocated = prizePot + treasuryBalance + totalUnclaimedPrizes + endgameOwed + dormancyOGPool + dormancyCasualRefundPool + dormancyCommitmentPool + dormancyPerHeadPool + tierPoolsTotal + currentDrawSeedReturn + resetDrawRefundPool + resetDrawRefundPool2 + commitmentRefundPool + totalForceDeclineRefundOwed + draw30BonusFund; // [v1.62]
        isSolvent = totalValue + SOLVENCY_TOLERANCE >= totalAllocated;
    }
    ...
    ///          Returns 0 at draw 30+ (surplus path used instead -- see @notice).
    ...
    ///                       Denominator is requiredEndPot (obligation * targetReturnBps/10000
    ///                       + DRAW30_PRIZE_RESERVE), not gross obligation.
    ...
    /// @return t1Min/t1Max         Acceptable T1 count range (0.5%-4% of snapshot).
    /// @return t2Min/t2Max         Acceptable T2 cumulative count range (4%-12% of snapshot). [v2.18: was 6%]
    /// @return t3Min/t3Max         Acceptable T3 cumulative count range (10%-50% of snapshot). [v2.18: was 16%]
    ...
    function getRequiredCutoffDiffBounds() external view returns (
        ...
        if (snapshot > 0) {
            // [v1.54] L-03 FIX: floor min values at 1 when snapshot is very small.
            // Integer division can return 0 for small snapshots (e.g. t1Min = 100 * 50 / 10000 = 0).
            // submitCutoffDiffs() enforces the BPS check authoritative -- but a returned min of 0
            // is misleading to keepers. Return at least 1 when snapshot > 0.
            t1Min = snapshot * T1_COUNT_MIN_BPS / 10000; if (t1Min == 0) t1Min = 1;
            t1Max = snapshot * T1_COUNT_MAX_BPS / 10000;
            t2Min = snapshot * T2_COUNT_MIN_BPS / 10000; if (t2Min == 0) t2Min = 1;
            t2Max = snapshot * T2_COUNT_MAX_BPS / 10000;
            t3Min = snapshot * T3_COUNT_MIN_BPS / 10000; if (t3Min == 0) t3Min = 1;
            t3Max = snapshot * T3_COUNT_MAX_BPS / 10000;
        }
        priceForDiffs = resolvedPrice;
    }

    /// @notice Returns the most recently resolved BTC/USD price (Chainlink 8-dec).
    ...
    function checkSolvency() external view returns (bool solvent, uint256 deficit) {
        ...
        uint256 curTargetBps = committedPlayerCount > 0
            ? _computeTargetReturnBps((maxOGs * 10000) / committedPlayerCount) : 9000;
        uint256 floor = obligation * curTargetBps / 10000 + DRAW30_PRIZE_RESERVE;
        uint256 casualCount = committedPlayerCount > maxOGs ? committedPlayerCount - maxOGs : 0;
    ...
    ///      Curve: 90% at <=20% OG, linear to 30% at 100% OG. Hard floor 30%.
    ...
    /// @return ret      targetReturnBps in BPS. Range [3000, 9000].
    function _computeTargetReturnBps(uint256 ratioBps) internal pure returns (uint256 ret) {
        if (ratioBps <= 2000) return 9000;
        uint256 reduction = (ratioBps - 2000) * 6000 / 8000;
        // When reduction < 9000: true branch returns 9000 - reduction (range [3000, ~9000]).
        // At 100% OG (ratioBps=10000): reduction=6000, returns 3000 via TRUE branch.
        // False branch (explicit 3000): dead code -- ratioBps <= 10000 so reduction <= 6000 < 9000.
        // Retained for clarity; the 3000 floor IS reachable, only the else branch is not.
        return reduction < 9000 ? 9000 - reduction : 3000;
    }

    // ── [v2.15] Deprecated treasury helpers ─────────────────────────────────

    /// @dev [DEPRECATED v2.15] Graduated treasury removed. Zero callers.
    ///      Always returns TREASURY_BPS (1500).
    ///      Retained for audit trail continuity. Safe to remove after Cyfrin audit
    ///      acceptance and before mainnet deployment.
    function _getDrawTreasuryBps() internal pure returns (uint256) {
        return TREASURY_BPS;
    }

    /// @dev [DEPRECATED v2.15] Graduated treasury removed. Zero callers.
    ///      Returns TREASURY_BPS (1500) for all draw values.
    ///      Retained for audit trail continuity. Safe to remove after Cyfrin audit
    ///      acceptance and before mainnet deployment.
    function _getHistoricalTreasuryBps(uint256 /*draw*/) internal pure returns (uint256) {
        return TREASURY_BPS;
    }
    ...
    ///      Anchors: 165 bps at tReturnBps=3000 (30% return, 100% OG load);
    ///               700 bps at tReturnBps=9000 (90% return, low OG load).
    ///      Linear interpolation between anchors. All paths clamped to [breathRailMin, breathRailMax].
    ///      [v1.90] Rail clamp unified -- early returns for <=3000 and >=9000 cases previously
    ///      bypassed the clamp. Now all three branches clamp unconditionally.
    ...
    {
        if (tReturnBps <= 3000)       bps = 165;
        else if (tReturnBps >= 9000)  bps = BREATH_START;
        else bps = 165 + (tReturnBps - 3000) * (BREATH_START - 165) / (9000 - 3000);
    ...
    function _finalReturnCalibration() internal {
        ...
        if (ogEndgameObligation > 0) { requiredEndPot = ogEndgameObligation * targetReturnBps / 10000 + DRAW30_PRIZE_RESERVE; }
    ...
    function _snapshotOGObligation() internal {
        ...
        uint256 newRequired = newObligation * targetReturnBps / 10000 + DRAW30_PRIZE_RESERVE;
    ...
    function _computeBalanceAndAllocated() internal view returns (uint256 actualBalance, uint256 nonPotAllocated) {
        ...
        nonPotAllocated = treasuryBalance + totalUnclaimedPrizes + endgameOwed
            ...
            + draw30BonusFund; // [v1.62] must be tracked or _captureYield sweeps it back into prizePot
    }
    ...
    function _calculatePrizePools() internal {
        ...
        if (currentDraw == TOTAL_DRAWS) {
            ...
            // [v1.03/v2.27] D-1: holdback changed from gross ($600/OG) to curve ceiling ($540/OG).
            // [v1.03/v2.29] Running-average: holdback uses 29-draw ogRatioBpsAccumulator estimate.
            // [v1.03/v2.30] TRUE SSoT: ogRatioDrawCount=29 at both holdback time and closeGame()
            // because _finalizeWeekCore() excludes draw 30 via currentDraw < TOTAL_DRAWS guard.
            ...
            uint256 holdback = obligationLocked ? ogEndgameObligation * estReturnBps / 10000 : 0;
    ...
    /// @dev [v1.57-P1] DEPRECATED dead code. Upfront OG cap removed in v1.57-P1.
    ///      Zero callers. Retained for audit trail continuity.
    ///      See _weeklyOGCapReached() for the enforced cap.
    ///      Presence in bytecode increases deployment gas by ~1,500-3,000 gas (bytecode overhead).
    ///      Safe to remove post-audit-acceptance. No side effects.
    ...
    function _readBtcPrice() internal view returns (int256) {
        AggregatorV3Interface feed = AggregatorV3Interface(btcFeed);
        try feed.latestRoundData() returns (uint80 roundId, int256 price, uint256, uint256 updatedAt, uint80 answeredInRound) {
            ...
            if (price <= 0) return 0;
            try AggregatorMinMax(btcFeed).minAnswer() returns (int192 minAns) { if (price <= int256(minAns)) return 0; } catch {}
            try AggregatorMinMax(btcFeed).maxAnswer() returns (int192 maxAns) { if (maxAns > 0 && price >= int256(maxAns)) return 0; } catch {}
            return price;
        } catch { return 0; }
    }
