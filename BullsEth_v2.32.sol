...
/**
 * @title  BullsEth v2.32  [contract renamed from NearestTheETH30_Base]
 * @notice 30-draw ETH/USD prediction game. 72h draw cycle, 90-day season.
 *         Fork of NearestTheETH_Base_1Y v1.86. All v1.86 audit fixes inherited.
 *         Audit fixes from v1.0 internal review: M-01, L-01, L-02, L-03 resolved.
 ...
 *               Brand: NearestTheETH30 -> BullsEth.
 ...
 *               NS-I-02 setReserveFeed/setWethFeed FeedSubstituted semantics comment.
 ...
 *
 *         v2.27: 0C/0H/1M/1L (code) + 2L NS + 1I IC. Cold-read triple audit.
 *               D-1 MEDIUM: draw-30 holdback used gross ogEndgameObligation (N x $600)
 *               but closeGame() only pays max N x $540 (9000 bps target return).
 *               The $60/OG over-holdback absorbed the DRAW30_PRIZE_RESERVE once
 *               N > ~83 OGs, collapsing the flagship draw-30 prize to just
 *               draw30BonusFund. Fix: holdback = ogEndgameObligation *
 *               MAX_TARGET_RETURN_BPS / 10000. MAX_TARGET_RETURN_BPS = 9000 is the
 *               hard ceiling of _computeTargetReturnBps() so avgTargetReturnBps can
 *               never exceed it. OGs cannot be underpaid; DRAW30_PRIZE_RESERVE is
 *               restored as intended. New constant MAX_TARGET_RETURN_BPS added.
 *               B-1 LOW: claimResetRefund() both pool paths decremented p.totalPaid
 *               by NET (claim) but totalOGPrincipal by GROSS (grossEquiv). After a
 *               reset refund, p.totalPaid was left with the treasury-slice residual
 *               (~15%), diverging from totalOGPrincipal. Fix: move grossEquiv
 *               computation before the totalPaid decrement; decrement both by GROSS.
 *               NS-2.21-01 carry: claimEndgame() @notice still said "guaranteed"
 *               (survived v1.63 language sweep). Fixed.
 *               NS-2: getProjectedEndgamePerOG() @return potHealth described
 *               denominator as "obligation" but code divides by requiredEndPot
 *               (obligation scaled by targetReturnBps + DRAW30_PRIZE_RESERVE).
 *               IC-1: WORST-CASE SCENARIOS runbook: draw-30 emergency reset path
 *               was undocumented. Added scenario 5: reset on draw 30 closes the
 *               season on refunds without a draw-30 prize distribution.
 *
 *         v2.28: Delta audit on v2.27. 0 new C/H/M. NS-A/NS-B/IC-C + D-2 doc.
 *               NS-A: v2.27 changelog entry was mis-ordered in header -- inserted
 *               after v2.20 instead of after v2.26. Moved to correct position.
 *               D-2 (Info): high-OG season holdback over-reserve documented in
 *               MAX_TARGET_RETURN_BPS @dev. At targetReturnBps < 9000 the holdback
 *               still uses 9000 -- unavoidable at draw-30 time. Optimal for the
 *               expected 5-20% OG regime. No code change.
 *               NS-B (Low): two "above OG obligation" summary comments updated to
 *               "above targeted OG holdback (MAX_TARGET_RETURN_BPS)" after D-1 fix.
 *               IC-C (Info): snapshotTotalEntries @dev added case (3): status-lost
 *               OG who buys as casual counts 3x in snapshot (2 ogList + 1 nonOG)
 *               but generates 1-2 entries. Same overcount direction as case (2).
 *
 *         v2.29: Running-average holdback. 1 code fix.
 *               D-2 PARTIALLY RESOLVED: _calculatePrizePools() draw-30 holdback
 *               now uses 29-draw ogRatioBpsAccumulator estimate instead of
 *               MAX_TARGET_RETURN_BPS ceiling. CORRECTION [v2.30]: the changelog
 *               claim that "ogRatioDrawCount=29 at closeGame() because
 *               _checkAutoAdjust() does not run on draw 30" was WRONG.
 *               _checkAutoAdjust() does not update ogRatioDrawCount at all.
 *               ogRatioDrawCount is updated in _finalizeWeekCore(), which DOES
 *               run on draw 30. At closeGame() time ogRatioDrawCount was 30, not
 *               29, causing a spurious EndgameShortfall in high-OG declining
 *               seasons ($88.50 false distress / 25 OGs). Fixed in v2.30.
 *
 *         v2.30: True single-source-of-truth. 1 code fix.
 *               Fix: _finalizeWeekCore() accumulator guard adds
 *               `&& currentDraw < TOTAL_DRAWS`. Draw 30's ratio is intentionally
 *               excluded from ogRatioBpsAccumulator. ogRatioDrawCount=29 at both
 *               draw-30 resolveWeek() holdback time AND at closeGame() time.
 *               Holdback and perOGPromised now use the exact same 29-draw figure
 *               by architecture, not by coincidence. Spurious EndgameShortfall
 *               in high-OG declining seasons eliminated. Integer dust only.
 *               5-20% OG design target: zero behavioral change (estReturnBps=9000).
 *               Confirmed: currentDraw++ runs AFTER the accumulator block in
 *               _finalizeWeekCore(), so the guard currentDraw < TOTAL_DRAWS is
 *               evaluated when currentDraw==30 on the final draw. Guard is false.
 *
 *         v2.31: NS/IC only. Zero bytecode change.
 *               NS-1 (L): ogRatioDrawCount @dev said "Typically 30 in normal
 *               operation." Wrong as of v2.30 guard. Now correctly 29.
 *               NS-2 (L): ogRatioBpsAccumulator @dev said "across all non-reset
 *               draws." Now "all non-reset draws except draw 30 (v2.30 SSoT guard)."  
 *               NS-3 (L): closeGame() @dev NOTE said "only normal draws count"
 *               without the draw-30 exclusion. Updated to match the body comment.
 *               NS-4 (I): MAX_TARGET_RETURN_BPS @dev D-2 tag re-pointed from v2.29
 *               to v2.30. "Matching closeGame() exactly" was false in v2.29 --
 *               only true after the v2.30 guard. Changelog reference corrected.
 *               NS-5 (I): v2.30 changelog line-number citations (L2542, L2535-2539)
 *               replaced with relational descriptions (line numbers drift with edits).
 *
 *         KEEPER TRUST MODEL:
 ...
 *           5. [v2.27] Emergency reset on draw 30: draw is voided, casuals refunded
 *              from resetDrawRefundPool, OG status restored via _continueUnwind().
 *              The TOTAL_DRAWS guard then fires in _finalizeWeekCore() setting
 *              gamePhase = CLOSED. Season ends WITHOUT a draw-30 prize distribution.
 *              No fund risk -- refunds and OG endgame claims remain available.
 *              No draw-30 replay: the game is permanently closed after reset.
 *
...
contract BullsEth is ReentrancyGuard, Ownable2Step {
    ...
    ///      In the 5-20% OG design target: estReturnBps = 9000 -- no behavioral change.
    ...
    uint256 public constant MAX_TARGET_RETURN_BPS    = 9000;  // must match _computeTargetReturnBps() hard ceiling
    ...
    /// @notice Deploys BullsEth_NoAave. Pure USDC contract -- no Aave dependency.
    ...
    ///      (v2.30 SSoT guard) -- only draws 1-29 are counted. See ogRatioDrawCount @dev.
    ...
    ///                       + DRAW30_PRIZE_RESERVE), not gross obligation. Monitoring tools should
    ///                       calibrate alerts against requiredEndPot, not the gross figure.
